...
void module_set_param(u32 idx, ParamValue v) {
  ...
  case eParam_delay0 :
    check_fade_rd(0);
    delayFadeN_set_delay_sec(&(lines[0]), v, 0 ); // fadeTargetRd[0]);
    break;
  case eParam_delay1 :
    check_fade_rd(1);
    delayFadeN_set_delay_sec(&(lines[1]), v, 0 ); // fadeTargetRd[1]);
    break;
  case eParam_loop0 :
  ...
  case eParam_pos_write0 :
    // check_fade_wr(0);
    delayFadeN_set_pos_write_sec(&(lines[0]), v, 0 ); // fadeTargetWr[0]);
    break;
  case eParam_pos_write1 :
    // check_fade_wr(1);
    delayFadeN_set_pos_write_sec(&(lines[1]), v, 0 ); // fadeTargetWr[1] );
    break;
  case eParam_pos_read0 :
    check_fade_rd(0);
    delayFadeN_set_pos_read_sec(&(lines[0]), v, 0 ); // fadeTargetRd[0]);
    break;
  case eParam_pos_read1 :
    check_fade_rd(1);
    delayFadeN_set_pos_read_sec(&(lines[1]), v, 0 ); // fadeTargetRd[1]);
    break;
  ...
  case eParam_rMul0 :
    check_fade_rd(0);
    delayFadeN_set_mul(&(lines[0]), v >> 16, 0 ); // fadeTargetRd[0]);
    break;
  case eParam_rDiv0 :
    check_fade_rd(0);
    delayFadeN_set_div(&(lines[0]), v >> 16, 0 ); // fadeTargetRd[0]);
    break;
  case eParam_rMul1 :
    check_fade_rd(1);
    delayFadeN_set_mul(&(lines[1]), v >> 16 , 0 ); // fadeTargetRd[1]);
    break;
  case eParam_rDiv1 :
    check_fade_rd(1);
    delayFadeN_set_div(&(lines[1]), v >> 16 , 0 ); // fadeTargetRd[1]);
    break;
  case eParam_write0 :
