...
static region sketchPad = { .w=128, .h=64, .x=0,  .y=0  };
//-------------------------
//---- static funcs
...
any doEtchaFill (any x) {
  any fillVal;
  fillVal = EVAL(cadr(x));
  if(isNum(fillVal)) {
    int fillValInt = unBox(fillVal) & 0xF;
    region_fill(&sketchPad, fillValInt);
    return T;
  }
  return Nil;
}

any doEtchaPixel (any x) {
  any xPos = EVAL(cadr(x));
  any yPos = EVAL(caddr(x));
  any pixVal = EVAL(cadddr(x));
  if(isNum(xPos) && isNum(yPos) && isNum(pixVal)) {
    *(sketchPad.data + (unBox(yPos) * 128 ) + unBox(xPos))
      = unBox(pixVal) & 0xF;
    sketchPad.dirty = 1;
    return T;
  }
