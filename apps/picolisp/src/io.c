...
/*** Reading ***/
void getStdin(void) {
  Chr = inputBuffer[inputIdx];
  if (inputBuffer[inputIdx] == 0) {
    Chr = -1;
  } else {
      inputIdx ++;
  }
...
any doStr(any ex) {
   ...
   begString();
   while (print_pl(car(x)), isCell(x = cdr(x)))
      space();
   return endString();
}

any load(int pr, any x) {
   cell c1, c2;
...
void print_pl(any x) {
   if (isNum(x))
      ...
      Env.put('\''),  print_pl(cdr(x));
   else {
      any y;

      Env.put('(');
      if ((y = circ(x)) == NULL) {
         while (print_pl(car(x)), !isNil(x = cdr(x))) {
            if (!isCell(x)) {
               outString(" . ");
               print_pl(x);
               break;
...
any doPrint(any x) {
   ...
   x = cdr(x),  print_pl(y = EVAL(car(x)));
   while (isCell(x = cdr(x)))
      space(),  print_pl(y = EVAL(car(x)));
   return y;
}
