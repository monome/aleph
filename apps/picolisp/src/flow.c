...
static void traceIndent(int i, any x, char *s) {
   ...
   if (!isCell(x))
      ...
      print_pl(car(x)), space(), print_pl(cdr(x)), space(), print_pl(val(This));
   outString(s);
}
...
any doTrace(any x) {
   ...
   for (x = car(x);  isCell(x);  x = cdr(x))
      space(), print_pl(val(car(x)));
   if (!isNil(x)) {
      if (x != At)
         space(), print_pl(val(x));
      else {
         int i = Env.next;

         while (--i >= 0)
            space(), print_pl(data(Env.arg[i]));
      }
   }
   newline();
   /* Env.put = putSave; */
   ...
   /* OutFile = stderr; */
   /* Env.put = putStdout; */
   traceIndent(Trace--, foo, " = "),  print_pl(data(c1)),  newline();
   /* Env.put = putSave; */
   /* OutFile = oSave; */
   return Pop(c1);
}
