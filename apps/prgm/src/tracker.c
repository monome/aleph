...
void init_tstep(tstep *t) {
    ...
    step_t->osc0_t = 1 * 0x00010000;
    step_t->osc1_t = 1 * 0x00010000;
    step_t->osc2_t = 1 * 0x00010000;
    step_t->osc3_t = 1 * 0x00010000;
    step_t = step_t->next;
}
    ...
    switch (state_sw) {
        case 0:
            ...
            if (touchedThis) {
                ...
                ctl_param_change(eParamCVfreq1, Freq2);
...
void step_advance(s32 val) {
    ...
    if(val == 0) { return; }
        ...
        if (pageIdx != ePagePrgm) { render_freq(); }
        gpio_set_gpio_pin(LED_MODE_PIN);
    }
    
    else if (counter < F_STEPS_MAX) {
        gpio_clr_gpio_pin(LED_MODE_PIN);
        tmp0 = Freq0;
        tmp1 = Freq1;
        tmp2 = Freq2;
...
void step_advance_t(s32 val) {
    ...
    if(val == 0) { return; }
    ...
    else if (counter_t < T_STEPS_MAX - 1) {
        gpio_clr_gpio_pin(LED_MODE_PIN);
        tmp0 = Transpose0;
        tmp1 = Transpose1;
        tmp2 = Transpose2;
        ...
        if (pageIdx != ePagePrgm) { render_transpose(); }
        gpio_set_gpio_pin(LED_MODE_PIN);
    }
    
    else if (counter_t < T_STEPS_MAX) {
        gpio_clr_gpio_pin(LED_MODE_PIN);
        tmp0 = Transpose0;
        tmp1 = Transpose1;
        tmp2 = Transpose2;
