...
fract32 pan_mix(fract32 inX, fract32 inY, fract32 pan) {
  fix16 xIdx = fix16_mul(FRACT_FIX16(pan), FADE_SINE_BUF_SIZE_FIX);
...
inline void pan_coeff(fract32* a, fract32* b, fract32 pan) {
  fix16 xIdx = fix16_mul(FRACT_FIX16(pan), FADE_SINE_BUF_SIZE_FIX);
  fix16 yIdx = fix16_sub(FADE_SINE_BUF_SIZE_FIX, xIdx);
  *a = table_lookup_idx(sineTab, FADE_SINE_BUF_SIZE, xIdx);
  *b = table_lookup_idx(sineTab, FADE_SINE_BUF_SIZE, yIdx);
}
