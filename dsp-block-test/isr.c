...
void sport0_rx_isr(void) {
  ...
  // FIXME: need to convert from 24b to 32b for fract32 DSP paths
  if(inBufFlag) {
...
#else
  u16 i;
  // copy input for new block (interleaved fractal32 DMA buffer)
  for(i=0; i<BLOCKSIZE; i++) {
    audioIn[0][i] = audioRxBuf[i * CHANNELS] << 8;
    audioIn[1][i] = audioRxBuf[i * CHANNELS + 1] << 8;
    audioIn[2][i] = audioRxBuf[i * CHANNELS + 2] << 8;
    audioIn[3][i] = audioRxBuf[i * CHANNELS + 3] << 8;
  }
#endif
...
#if DMA_DEINTERLEAVE_PINGPONG
  // FIXME: need to convert from 32b to 24b for fract32 DSP paths
  if(outBufFlag) {
...
#else
  u16 i;
  // copy output from last block (interleaved fractal32 DMA buffer)
  for(i=0; i<BLOCKSIZE; i++) {
    audioTxBuf[i * CHANNELS] = audioOut[0][i] >> 8;
    audioTxBuf[i * CHANNELS + 1] = audioOut[1][i] >> 8;
    audioTxBuf[i * CHANNELS + 2] = audioOut[2][i] >> 8;
    audioTxBuf[i * CHANNELS + 3] = audioOut[3][i] >> 8;
  }
#endif
