...
extern u8 ctl_param_change(u32 step, u32 idx, u32 val) {
    u32 i;
...
extern void ctl_do_last_change(void) {
    static u32 step;
    static u32 idx;
    step = ctlBuf[evCount -1].step;
    idx = ctlBuf[evCount - 1].idx;
    module_set_sqparam(step, idx, ctlBuf[evCount - 1].val);
...
extern void ctl_perform_last_change(void) {
    ...
    static s32 idx;
    if(evCount == 0) { return; }
    step = ctlBuf[--evCount].step;
    idx = ctlBuf[--evCount].idx;
    module_set_sqparam(step, idx, ctlBuf[evCount].val);
    clear_param_dirty(idx);
}
