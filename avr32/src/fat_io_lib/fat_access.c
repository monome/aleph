...
int fatfs_init(struct fatfs *fs) {
    ...
    if (!fs->disk_io.read_media) return FAT_INIT_MEDIA_ACCESS_ERROR;
    ...
    */
    ...
    // Now check again using the access function to prove endian conversion
    // function
    ...
    switch (fs->currentsector.sector[PARTITION1_TYPECODE_LOCATION]) {
        ...
        case 0x0E:
        case 0x0F:
        case 0x05: valid_partition = 1; break;
        case 0x00: valid_partition = 0; break;
        default:
    ...
    fs->rootdir_first_sector =
        reserved_sectors + (num_of_fats * fs->fat_sectors);
    fs->rootdir_sectors =
        ((fs->root_entry_count * 32) + (FAT_SECTOR_SIZE - 1)) / FAT_SECTOR_SIZE;
    ...
    if (GET_16BIT_WORD(fs->currentsector.sector, 0x1FE) !=
        0xAA55)  // This signature should be AA55
        return FAT_INIT_INVALID_SIGNATURE;

    // Calculate the root dir sectors
    root_dir_sectors =
        ((GET_16BIT_WORD(fs->currentsector.sector, BPB_ROOTENTCNT) * 32) +
...
// fatfs_sector_read:
//-----------------------------------------------------------------------------
int fatfs_sector_read(struct fatfs *fs, uint32 lba, uint8 *target,
                      uint32 count) {
...
// fatfs_sector_write:
//-----------------------------------------------------------------------------
int fatfs_sector_write(struct fatfs *fs, uint32 lba, uint8 *target,
                       uint32 count) {
    return fs->disk_io.write_media(lba, target, count);
}
//-----------------------------------------------------------------------------
// fatfs_sector_reader: From the provided startcluster and sector offset
// Returns True if success, returns False if not (including if read out of
// range)
//-----------------------------------------------------------------------------
int fatfs_sector_reader(struct fatfs *fs, uint32 start_cluster, uint32 offset,
                        uint8 *target) {
    ...
    // Else read sector if not already loaded
        ...
        return fs->disk_io.read_media(fs->currentsector.address,
...
//-----------------------------------------------------------------------------
int fatfs_read_sector(struct fatfs *fs, uint32 cluster, uint32 sector,
                      uint8 *target) {
    // FAT16 Root directory
    ...
    // FAT16/32 Other
        ...
        if (target) {
            ...
            fs->currentsector.address =
                fatfs_lba_of_cluster(fs, cluster) + sector;
...
int fatfs_write_sector(struct fatfs *fs, uint32 cluster, uint32 sector,
                       uint8 *target) {
    // No write access?
    if (!fs->disk_io.write_media) return 0;
    ...
    if (fs->fat_type == FAT_TYPE_16 && cluster == 0) {
        ...
        if (target) {
            ...
            // Write to disk
            return fs->disk_io.write_media(fs->currentsector.address,
                                           fs->currentsector.sector, 1);
        }
    }
    // FAT16/32 Other
...
void fatfs_show_details(struct fatfs *fs) {
    ...
    /* FAT_PRINTF((" Type =%s", (fs->fat_type == FAT_TYPE_32) ? "FAT32":
     * "FAT16")); */
...
uint32 fatfs_get_file_entry(struct fatfs *fs, uint32 Cluster,
    ...
    while (1) {
        // Read sector
        if (fatfs_sector_reader(fs, Cluster, x++,
                                0))  // If sector read was successfull
        {
            ...
            for (item = 0; item < FAT_DIR_ENTRIES_PER_SECTOR; item++) {
                ...
                directoryEntry =
                    (struct fat_dir_entry *)(fs->currentsector.sector +
                                             recordoffset);
                ...
                if (fatfs_entry_lfn_text(directoryEntry))
                    fatfs_lfn_cache_entry(
                        &lfn, fs->currentsector.sector + recordoffset);

                // If Invalid record found delete any long file name information
                // collated
                ...
                // Normal SFN Entry and Long text exists
                    ...
                    for (i = 8; i < 11; i++) {
                        ...
                        if (directoryEntry->Name[i] != ' ') dotRequired = 1;
                    }
                                ...
                                0))  // If sector read was successfull
        {
            ...
            for (item = 0; item < FAT_DIR_ENTRIES_PER_SECTOR; item++) {
                ...
                // collated
                    ...
                    // Normal Entry, only 8.3 Text
                    if (fatfs_entry_sfn_only(directoryEntry)) {
                    if (strncmp((const char *)directoryEntry->Name, shortname,
                                11) == 0)
                        return 1;
                }
            }  // End of if
...
//-------------------------------------------------------------
#if FATFS_INC_WRITE_SUPPORT
int fatfs_update_file_length(struct fatfs *fs, uint32 Cluster, char *shortname,
                             uint32 fileLength) {
    uint8 item = 0;
    uint16 recordoffset = 0;
    int x = 0;
            ...
            for (item = 0; item < FAT_DIR_ENTRIES_PER_SECTOR; item++) {
                ...
                // Normal Entry, only 8.3 Text
                    ...
                    if (strncmp((const char *)directoryEntry->Name, shortname,
                        ...
                        // Update sfn entry
                        memcpy(
                            (uint8 *)(fs->currentsector.sector + recordoffset),
                            (uint8 *)directoryEntry,
                            sizeof(struct fat_dir_entry));

                        // Write sector back
                        return fs->disk_io.write_media(
                            fs->currentsector.address, fs->currentsector.sector,
                            1);
                    }
                }
            }  // End of if
...
int fatfs_list_directory_next(struct fatfs *fs,
    ...
    while (1) {
        ...
        if (fatfs_sector_reader(fs, dirls->cluster, dirls->sector, 0)) {
            ...
            for (item = dirls->offset; item < FAT_DIR_ENTRIES_PER_SECTOR;
                 item++) {
                ...
                // Normal SFN Entry and Long text exists
                    ...
                    // Get text
                    long_filename = fatfs_lfn_cache_get(&lfn);
                    strncpy(entry->filename, long_filename,
                            FATFS_MAX_LONG_FILENAME - 1);
                    ...
                    entry->cluster =
                        (FAT_HTONS(directoryEntry->FstClusHI) << 16) |
                        FAT_HTONS(directoryEntry->FstClusLO);
