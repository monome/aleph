...
int fatfs_add_free_space(struct fatfs *fs, uint32 *startCluster,
    ...
    for (i = 0; i < clusters; i++) {
        ...
        if (fatfs_find_blank_cluster(fs, fs->rootdir_first_cluster,
                                     &nextcluster)) {
            // Point last to this
            ...
            start = nextcluster;
            if (i == 0) *startCluster = nextcluster;
        } else
            return 0;
    }
...
// newFile = true.
//-----------------------------------------------------------------------------
int fatfs_allocate_free_space(struct fatfs *fs, int newFile,
    ...
    // If any left over
    if (size - (clusterSize * clusterCount)) clusterCount++;
    ...
    if (!fatfs_add_free_space(fs, &nextcluster, clusterCount)) return 0;
...
//-----------------------------------------------------------------------------
static int fatfs_find_free_dir_offset(struct fatfs *fs, uint32 dirCluster,
                                      int entryCount, uint32 *pSector,
                                      uint8 *pOffset) {
    ...
    while (1) {
        ...
        if (fatfs_sector_reader(fs, dirCluster, x++, 0)) {
            ...
            for (item = 0; item < FAT_DIR_ENTRIES_PER_SECTOR; item++) {
                ...
                // SFN Entry
                    ...
                    if (fs->currentsector.sector[recordoffset] ==
                        FILE_HEADER_DELETED) {
                        // First entry?
                        if (possible_spaces == 0) {
                            // Store start
                        ...
                        if (possible_spaces >= entryCount) return 1;

                        // Else continue counting until we find a valid entry!
                    }
                    // Is the file entry empty?
                    else if (fs->currentsector.sector[recordoffset] ==
                             FILE_HEADER_BLANK) {
                        // First entry?
                        if (possible_spaces == 0) {
                            // Store start
        ...
        // Run out of free space in the directory, allocate some more
            ...
            for (i = 0; i < fs->sectors_per_cluster; i++) {
                if (!fatfs_write_sector(fs, newCluster, i, 0)) return 0;
            }
...
//-----------------------------------------------------------------------------
// fatfs_add_file_entry: Add a directory entry to a location found by
// FindFreeOffset
//-----------------------------------------------------------------------------
int fatfs_add_file_entry(struct fatfs *fs, uint32 dirCluster, char *filename,
                         char *shortfilename, uint32 startCluster, uint32 size,
                         int dir) {
    uint8 item = 0;
    uint16 recordoffset = 0;
    uint8 i = 0;
    ...
    // NOTE: We need to find space for at least the LFN + SFN (or just the SFN
    // if LFNs not supported).
    if (!fatfs_find_free_dir_offset(fs, dirCluster, entryCount + 1, &dirSector,
                                    &dirOffset))
        return 0;
        ...
        if (fatfs_sector_reader(fs, dirCluster, x++, 0)) {
            ...
            for (item = 0; item < FAT_DIR_ENTRIES_PER_SECTOR; item++) {
                ...
                if (foundEnd) {
                    if (entryCount == 0) {
                        // Short filename
                        fatfs_sfn_create_entry(shortfilename, size,
                                               startCluster, &shortEntry, dir);
                        memcpy(&fs->currentsector.sector[recordoffset],
                               &shortEntry, sizeof(shortEntry));
                        ...
                        fatfs_filename_to_lfn(
                            filename, &fs->currentsector.sector[recordoffset],
                            entryCount, checksum);
                        dirtySector = 1;
                    }
#endif
            ...
            if (dirtySector) {
                if (!fs->disk_io.write_media(fs->currentsector.address,
