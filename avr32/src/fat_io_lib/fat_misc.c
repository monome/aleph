...
void fatfs_lfn_cache_entry(struct lfn_cache *lfn, uint8 *entryBuffer) {
    ...
    if (LFNIndex > MAX_LONGFILENAME_ENTRIES) return;

    // This is an error condition
    if (LFNIndex == 0) return;

    if (lfn->no_of_strings == 0) lfn->no_of_strings = LFNIndex;
...
//-----------------------------------------------------------------------------
#if FATFS_INC_LFN_SUPPORT
void fatfs_filename_to_lfn(char *filename, uint8 *buffer, int entry,
                           uint8 sfnChk) {
    int i;
    int nameIndexes[MAX_LFN_ENTRY_LENGTH] = {
    ...
    // LFN entry number
    buffer[0] = (uint8)(((entriesRequired - 1) == entry) ? (0x40 | (entry + 1))
                                                         : (entry + 1));
...
void fatfs_sfn_create_entry(char *shortfilename, uint32 size,
                            uint32 startCluster, struct fat_dir_entry *entry,
                            int dir) {
    int i;

    // Copy short filename
    for (i = 0; i < FAT_SFN_SIZE_FULL; i++) entry->Name[i] = shortfilename[i];
...
int fatfs_lfn_create_sfn(char *sfn_output, char *filename) {
    ...
    if (dotPos != -1) {
        ...
        for (i = (dotPos + 1); i < (dotPos + 1 + 3); i++)
            if (i < len) ext[i - (dotPos + 1)] = filename[i];
    ...
    for (i = 0; i < len; i++) {
        ...
        if (pos == FAT_SFN_SIZE_PARTIAL) break;
    }

    // Add extension part
    for (i = FAT_SFN_SIZE_PARTIAL; i < FAT_SFN_SIZE_FULL; i++) {
        if (ext[i - FAT_SFN_SIZE_PARTIAL] >= 'a' &&
            ext[i - FAT_SFN_SIZE_PARTIAL] <= 'z')
...
static void fatfs_itoa(uint32 num, char *s) {
    ...
    cp = outbuf;
    do { *cp++ = digits[(int)(num % 10)]; } while ((num /= 10) > 0);

    *cp-- = 0;

    // Copy in forwards
    while (cp >= outbuf) *s++ = *cp--;
...
int fatfs_lfn_generate_tail(char *sfn_output, char *sfn_input, uint32 tailNum) {
    ...
    memcpy(sfn_output + (FAT_SFN_SIZE_PARTIAL - tail_chars), tail_str,
           tail_chars);
