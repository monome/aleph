...
void fatfs_fat_init(struct fatfs *fs) {
    ...
    for (i = 0; i < FAT_BUFFERS; i++) {
        ...
        memset(fs->fat_buffers[i].sector, 0x00,
               sizeof(fs->fat_buffers[i].sector));
    ...
    if (pcur) {
        ...
        if (pcur->dirty) {
            if (fs->disk_io.write_media) {
                ...
                if (!fs->disk_io.write_media(pcur->address, pcur->sector,
                                             sectors))
                    return 0;
            }
...
//-----------------------------------------------------------------------------
static struct fat_buffer *fatfs_fat_read_sector(struct fatfs *fs,
                                                uint32 sector) {
    ...
    while (pcur) {
        ...
        if ((sector >= pcur->address) &&
            (sector < (pcur->address + FAT_BUFFER_SECTORS)))
            break;
    ...
    if (pcur) {
        pcur->ptr = (uint8 *)(pcur->sector +
                              ((sector - pcur->address) * FAT_SECTOR_SIZE));
        return pcur;
    }
    ...
    // Writeback sector if changed
    if (pcur->dirty)
        if (!fatfs_fat_writeback(fs, pcur)) return 0;

    // Address is now new sector
    pcur->address = sector;

    // Read next sector
    if (!fs->disk_io.read_media(pcur->address, pcur->sector,
                                FAT_BUFFER_SECTORS)) {
...
uint32 fatfs_find_next_cluster(struct fatfs *fs, uint32 current_cluster) {
    ...
    if (!pbuf) return (FAT32_LAST_CLUSTER);
...
//-----------------------------------------------------------------------------
// fatfs_set_fs_info_next_free_cluster: Write the next free cluster to the
// FSINFO table
//-----------------------------------------------------------------------------
void fatfs_set_fs_info_next_free_cluster(struct fatfs *fs, uint32 newValue) {
    if (fs->fat_type == FAT_TYPE_16)
        ...
        struct fat_buffer *pbuf =
            fatfs_fat_read_sector(fs, fs->lba_begin + fs->fs_info_sector);
        if (!pbuf) return;
...
int fatfs_find_blank_cluster(struct fatfs *fs, uint32 start_cluster,
                             uint32 *free_cluster) {
    uint32 fat_sector_offset, position;
    uint32 nextcluster;
    ...
    do {
        ...
        if (fat_sector_offset < fs->fat_sectors) {
            // Read FAT sector into buffer
            pbuf = fatfs_fat_read_sector(fs,
                                         fs->fat_begin_lba + fat_sector_offset);
            if (!pbuf) return 0;

            if (fs->fat_type == FAT_TYPE_16) {
                // Find 32 bit entry of current sector relating to cluster
                // number
                position = (current_cluster - (fat_sector_offset * 256)) * 2;
            ...
            if (nextcluster != 0) current_cluster++;
        } else
...
#endif
//-----------------------------------------------------------------------------
// fatfs_fat_add_cluster_to_chain: Follow a chain marking and then add a new
// entry
// to the current tail.
//-----------------------------------------------------------------------------
#if FATFS_INC_WRITE_SUPPORT
int fatfs_fat_add_cluster_to_chain(struct fatfs *fs, uint32 start_cluster,
                                   uint32 newEntry) {
    uint32 last_cluster = FAT32_LAST_CLUSTER;
    uint32 next_cluster = start_cluster;

    if (start_cluster == FAT32_LAST_CLUSTER) return 0;
...
uint32 fatfs_count_free_clusters(struct fatfs *fs) {
    ...
    for (i = 0; i < fs->fat_sectors; i++) {
        ...
        for (j = 0; j < FAT_SECTOR_SIZE;) {
            if (fs->fat_type == FAT_TYPE_16) {
                if (FAT16_GET_16BIT_WORD(pbuf, (uint16)j) == 0) count++;

                j += 2;
            } else {
                if (FAT32_GET_32BIT_WORD(pbuf, (uint16)j) == 0) count++;
