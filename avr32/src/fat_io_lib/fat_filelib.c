...
///// this copy of fat_io_lib was hacked up on dec 31, 2012 by emb
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#include <stdlib.h>
#include <string.h>
...
#define FL_LOCK(a)                        \
    do {                                  \
        if ((a)->fl_lock) (a)->fl_lock(); \
    } while (0)
#define FL_UNLOCK(a)                          \
    do {                                      \
        if ((a)->fl_unlock) (a)->fl_unlock(); \
    } while (0)
...
static FL_FILE *_allocate_file(void) {
    ...
    // Add to open list
    if (node) fat_list_insert_last(&_open_file_list, node);
...
static int _open_directory(char *path, uint32 *pathCluster) {
    ...
    for (sublevel = 0; sublevel < (levels + 1); sublevel++) {
        if (fatfs_get_substring(path, sublevel, currentfolder,
                                sizeof(currentfolder)) == -1)
            return 0;
        ...
        if (fatfs_get_file_entry(&_fs, startcluster, currentfolder, &sfEntry)) {
            ...
            if (fatfs_entry_is_dir(&sfEntry))
                startcluster = ((FAT_HTONS((uint32)sfEntry.FstClusHI)) << 16) +
...
static int _create_directory(char *path) {
    ...
    if (fatfs_split_path((char *)path, file->path, sizeof(file->path),
    ...
    for (i = 0; i < _fs.sectors_per_cluster; i++) {
        if (!fatfs_write_sector(&_fs, file->startcluster, i,
                                file->file_data_sector)) {
            _free_file(file);
            return 0;
        }
    }
    ...
    do {
        ...
        if (tailNum != 0)
            fatfs_lfn_generate_tail((char *)file->shortfilename, shortFilename,
                                    tailNum);
        ...
        if (fatfs_sfn_exists(&_fs, file->parentcluster,
                             (char *)file->shortfilename) == 0)
            break;
    ...
    // Add file to disk
    if (!fatfs_add_file_entry(&_fs, file->parentcluster, (char *)file->filename,
                              (char *)file->shortfilename, file->startcluster,
                              0, 1)) {
        // Delete allocated space
        fatfs_free_cluster_chain(&_fs, file->startcluster);
...
static FL_FILE *_open_file(const char *path) {
    ...
    if (fatfs_get_file_entry(&_fs, file->parentcluster, file->filename,
        ...
        if (fatfs_entry_is_file(&sfEntry)) {
            ...
            file->bytenum = 0;
            file->startcluster =
                ((FAT_HTONS((uint32)sfEntry.FstClusHI)) << 16) +
                FAT_HTONS(sfEntry.FstClusLO);
            file->file_data_address = 0xFFFFFFFF;
            file->file_data_dirty = 0;
            file->filelength_changed = 0;
...
static FL_FILE *_create_file(const char *filename) {
    ...
    // No write access?
    if (!_fs.disk_io.write_media) return NULL;
    ...
    // Split full path into filename and directory path
    if (fatfs_split_path((char *)filename, file->path, sizeof(file->path),
                         file->filename, sizeof(file->filename)) == -1) {
        _free_file(file);
        return NULL;
    }
...
//-----------------------------------------------------------------------------
static uint32 _read_sectors(FL_FILE *file, uint32 offset, uint8 *buffer,
                            uint32 count) {
    uint32 Sector = 0;
    uint32 ClusterIdx = 0;
    uint32 Cluster = 0;
...
// fl_attach_locks:
//-----------------------------------------------------------------------------
void fl_attach_locks(struct fatfs *fs, void (*lock)(void),
                     void (*unlock)(void)) {
    fs->fl_lock = lock;
    fs->fl_unlock = unlock;
}
...
int fl_attach_media(fn_diskio_read rd, fn_diskio_write wr) {
    ...
    if ((res = fatfs_init(&_fs)) != FAT_INIT_OK) {
        //        FAT_PRINTF(("FAT_FS: Error could not load FAT details
        //        (%d)!\r\n", res));
...
void *fl_fopen(const char *path, const char *mode) {
    ...
    //        anywhere in the file for reading, but writing operations
...
//-----------------------------------------------------------------------------
#if FATFS_INC_WRITE_SUPPORT
static uint32 _write_sectors(FL_FILE *file, uint32 offset, uint8 *buf,
                             uint32 count) {
...
void fl_fclose(void *f) {
    ...
    if (file) {
        ...
        if (file->filelength_changed) {
            ...
            fatfs_update_file_length(&_fs, file->parentcluster,
                                     (char *)file->shortfilename,
                                     file->filelength);
#endif
            file->filelength_changed = 0;
...
int fl_fread(void *buffer, int size, int length, void *f) {
    ...
    while (bytesRead < count) {
        ...
        if ((offset == 0) && ((count - bytesRead) >= FAT_SECTOR_SIZE)) {
            ...
            uint32 sectorsRead = _read_sectors(
                file, sector, (uint8 *)((uint8 *)buffer + bytesRead),
            ...
            memcpy((uint8 *)((uint8 *)buffer + bytesRead),
                   (uint8 *)(file->file_data_sector + offset), copyCount);
...
int fl_fwrite(const void *data, int size, int count, void *f) {
    ...
    while (bytesWritten < length) {
        ...
        if ((offset == 0) && ((length - bytesWritten) >= FAT_SECTOR_SIZE)) {
            ...
            sectorsWrote =
                _write_sectors(file, sector, (uint8 *)(buffer + bytesWritten),
            ...
            memcpy((uint8 *)(file->file_data_sector + offset),
                   (uint8 *)(buffer + bytesWritten), copyCount);
...
int fl_remove(const char *filename) {
    ...
    if (file) {
        ...
        if (fatfs_free_cluster_chain(&_fs, file->startcluster)) {
            ...
            if (fatfs_mark_file_deleted(&_fs, file->parentcluster,
                                        (char *)file->shortfilename)) {
                // Close the file handle (this should not write anything to the
                // file
    ...
    if (fl_opendir(path, &dirstat)) {
        ...
        while (fl_readdir(&dirstat, &dirent) == 0) {
            if (dirent.is_dir) {
                ...
                // dirent.filename, dirent.cluster));
            } else {
...
FL_DIR *fl_opendir(const char *path, FL_DIR *dir) {
    ...
    if (levels == 0) cluster = fatfs_get_root_cluster(&_fs);
    // Find parent directory start cluster
    else
        res = _open_directory((char *)path, &cluster);

    if (res) fatfs_list_directory_start(&_fs, dir, cluster);
