...
// irq for PA24-PA31
__attribute__((__interrupt__)) static void irq_port0_line3(void);

// irq for PB00-PB07
__attribute__((__interrupt__)) static void irq_port1_line0(void);

// irq for PB08-PB15
__attribute__((__interrupt__)) static void irq_port1_line1(void);

// irq for PB16-PB23
__attribute__((__interrupt__)) static void irq_port1_line2(void);

// irq for PB24-PB31
__attribute__((__interrupt__)) static void irq_port1_line3(void);

// irq for uart
__attribute__((__interrupt__)) static void irq_usart(void);
...
__attribute__((__interrupt__)) static void irq_port0_line3(void) {
    ...
    if (gpio_get_pin_interrupt_flag(SW0_PIN)) {
        ...
        /// so clear the flag first to avoid triggering an infinite series of
        /// interrupts.
        /// this might be problematic if we were expecting faster interrupts
        /// from switches,
...
void register_interrupts(void) {
    ...
    // PA24 - PA31
    INTC_register_interrupt(&irq_port0_line3,
                            AVR32_GPIO_IRQ_0 + (AVR32_PIN_PA24 / 8),
                            UI_IRQ_PRIORITY);

    // PB00 - PB07
    INTC_register_interrupt(&irq_port1_line0,
                            AVR32_GPIO_IRQ_0 + (AVR32_PIN_PB00 / 8),
                            UI_IRQ_PRIORITY);
    ...
    // PB16 - PB23
    INTC_register_interrupt(&irq_port1_line2,
                            AVR32_GPIO_IRQ_0 + (AVR32_PIN_PB16 / 8),
                            UI_IRQ_PRIORITY);

    // PB24 - PB31
    INTC_register_interrupt(&irq_port1_line3,
                            AVR32_GPIO_IRQ_0 + (AVR32_PIN_PB24 / 8),
                            UI_IRQ_PRIORITY);
