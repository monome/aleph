...
__attribute__((__section__(".flash_nvram"))) nvram_data_t flash_nvram_data;
...
/////////// TESTING
  ...
  bfinLdrData = alloc_mem(BFIN_LDR_MAX_BYTES);
    ...
    if (flash_nvram_data.firstRun != FIRSTRUN_MAGIC) {
        ...
        bfinLdrSize = 0;
        flashc_memset32((void*)&(flash_nvram_data.ldrSize), 0x00000000, 4,
                        true);
        // do this only after succesful app launch
        //    flashc_memset32((void*)&(flash_nvram_data.firstRun),
        //    FIRSTRUN_MAGIC, 4, true);
        return 1;
    } else {
...
void flash_write_ldr(void) {
    //  flashc_memset32((void*)&(flash_nvram_data.ldrSize), bfinLdrSize, 4,
    //  true);
    //  flashc_memcpy((void*)&(flash_nvram_data.ldrData), (const
    //  void*)bfinLdrData, bfinLdrSize, true);
...
void flash_write_firstrun(void) {
    flashc_memset32((void*)&(flash_nvram_data.firstRun), FIRSTRUN_MAGIC, 4,
                    true);
}
...
void* flash_app_data(void) { return (void*)(&(flash_nvram_data.appData)); }
